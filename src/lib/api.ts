...
// バックエンド（/api）経由で Google Sheets と同期する薄いクライアント。
// サーバーが無い / スプレッドシート未設定なら enabled=false になり、
...
/** スプレッドシート連携が有効かを判定（サーバー未起動なら false） */
export async function checkSync(): Promise<boolean> {
  try {
    const h = await req<{ sheets: boolean }>('/health')
    return Boolean(h.sheets)
  } catch {
    return false
  }
}
